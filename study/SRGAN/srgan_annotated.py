...
class RegularizationLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        return loss
# ??: ??? WeightedPixelL1Loss
class WeightedPixelL1Loss(nn.Module):
    # ??: ??????????
    """加权L1：w = 1 + alpha * gray(target)"""
    # ??: ???? __init__
    ...
    def forward(self, pred, target):
        ...
        # ??: ?????????
        weight = 1.0 + self.alpha * target_gray
        # ??: ????
...
def validate_and_save(result_dir, generator, val_dataloader, device, epoch, data_type):
    ...
    # ??: ???? _convert_fake_for_display_by_hparam
