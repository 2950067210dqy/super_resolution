...
def _save_flow_error_visuals(
    ...
    _save_signed_error_map(du, out_dir / "aee_u_error.png", title_text="AEE-u Error", colorbar_label="u error [px]", stat_text=aee_text)
    _save_signed_error_map(dv, out_dir / "aee_v_error.png", title_text="AEE-v Error", colorbar_label="v error [px]", stat_text=aee_text)
    _save_signed_error_map(dw, out_dir / "aee_w_error.png", title_text="AEE-w Error", colorbar_label="w error [px]", stat_text=aee_text)
    _save_error_histogram(du, out_dir / "delta_u_error.png", title_text=r"$\Delta u$ Error Distribution", xlabel=r"$\Delta u$ displacement [px]", color="#F4A142", category_name=category_name)
    _save_error_histogram(dv, out_dir / "delta_v_error.png", title_text=r"$\Delta v$ Error Distribution", xlabel=r"$\Delta v$ displacement [px]", color="#4C9F70", category_name=category_name)
    _save_error_histogram(dw, out_dir / "delta_w_error.png", title_text=r"$\Delta w$ Error Distribution", xlabel=r"$\Delta w$ displacement [px]", color="#8E6BBE", category_name=category_name)
