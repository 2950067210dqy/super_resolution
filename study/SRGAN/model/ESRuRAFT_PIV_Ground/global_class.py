...
class global_data:
    class esrgan:
        ...
        DESCRIPTION = "v_bicubic_raft"  # 实验补充描述（可写损失配置、数据版本等）
        name +=DESCRIPTION
        ...
        # 3. "hr_ground_raft_load_models": 与 hr_ground_raft 使用同样的 HR 输入，
        #    但会先从 RAFT_CHECKPOINT 下的 checkpoint 导入 RAFT 权重。
        # 4. "bicubic_raft": 把 LR previous/next 用 bicubic 双三次插值放大到 HR 尺寸后送入 RAFT。
        # 5. "esrgan_raft": 使用原始 ESRGAN 生成器把 LR 超分到 HR，再把 SR 图像送入 RAFT。
        ...
        TRAIN_MODE_ALIASES = {
            ...
            "hr_ground_load_models": "hr_ground_raft_load_models",
            "bicubic": "bicubic_raft",
            "esrgan": "esrgan_raft",
            "bicubic_WIDIM": "bicubic_widim",
            "bicubic_HS": "bicubic_hs",
        }
        TRAIN_MODE = "bicubic_raft"
        ...
        # TRAIN_MODE="hr_ground_raft_load_models" 时使用的 RAFT checkpoint。
        # 默认沿用 RAFT_CHECKPOINT/ckpt_256.tar；如果要测试别的 RAFT 权重，只改这个路径即可。
        # 这里执行的是完整 RAFT 权重导入，和 RAFT128 的“部分预训练初始化”不是同一个概念。
        HR_GROUND_RAFT_LOAD_MODELS_CKPT = "RAFT_CHECKPOINT/ckpt_256.tar"
        # 默认严格加载，确保实际使用的 RAFT 结构与 checkpoint 完全匹配。
        # 如果 RAFT_MODEL_TYPE 与 checkpoint 不一致，会直接报清楚的结构错误。
        HR_GROUND_RAFT_LOAD_MODELS_STRICT = True
        ...
        # HS_EPS 只用于分母数值保护；不会改变 Horn-Schunck 的理论项，只避免低纹理区域出现除 0。
        HS_ALPHA = 1.0
        HS_ITERS = 100
        HS_EPS = 1e-6
