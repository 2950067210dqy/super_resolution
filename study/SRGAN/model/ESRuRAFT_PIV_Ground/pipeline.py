...
def _get_class_names_compat(gr_data_root_dir: str, dataset_name: str):
    ...
    某些运行环境里如果 `pipeline.py` 已经同步到新版本，但 `data_load.py` 仍是旧版本，
    直接传 `dataset_name=...` 会报 `unexpected keyword argument`。
    这里先尝试新签名，失败后：
    - class_1: 回退到旧签名；
    - class_2: 返回伪类别名，并在后续 `load_data` 阶段继续要求新版本数据加载逻辑。
    """
    try:
...
def _load_data_compat(**kwargs):
    ...
    - class_1: 如果当前环境导入到的是旧版 `load_data`，自动去掉新加的 class_2 相关关键字再调用。
    - class_2: 如果 `load_data` 仍是旧版，则直接给出明确报错，提示需要同步 `data_load.py`。
    """
    try:
...
def _trainable_parameters_or_dummy(module: nn.Module, device: torch.device):
    ...
    下 Generator/Discriminator 是 nn.Identity，没有任何参数。
