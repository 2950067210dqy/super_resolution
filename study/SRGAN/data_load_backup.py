...
def load_data(
    ...
    """
    加载超分辨率 GR/LR 成对数据集。会转成灰度图

    训练/验证集划分方式保持不变：仍然是按类别分别划分。
    改动点只是每个样本现在同时包含 GR 和 LR 两份数据。
    所以同样是 2 x 64 x 64，ti_pair 和 flo 的语义并不一样。
    第 1 个通道：第 1 张 ti 灰度图
    第 2 个通道：第 2 张 ti 灰度图
    如果样本类型是 flo，它也是 2 通道，但含义不同：

    通道 0：光流水平分量
    通道 1：光流垂直分量
    ti_pair -> [frame1, frame2]
    flo -> [u, v]
    """
    if not gr_data_root_dir or not lr_data_root_dir:
        raise ValueError("Please pass both gr_data_root_dir and lr_data_root_dir.")
