...
def _global_positive_colorbar_limit(attr_name: str, default: float) -> float:
    """
    从 PIV_A_Esrgan_Ablation 的全局超参数读取误差图色条半幅范围，并规范成正数。

    例如 FLOW_ERROR_COLORBAR_LIMIT=0.5 表示误差图显示范围为 [-0.5, 0.5]。
    该函数只影响 PNG 的显示色条，不改变原始误差 NPY、EPE/AEE、ESMSE 等评价指标。
    """
    try:
...
def evaluate_all(
    ...
    # 按用户要求不受 IS_SAVE_NPY 控制，因此 Ablation 分支也直接强制保存。
    if image_pair_rows:
        save_energy_spectrum_mse_compare_npy(
            image_pair_rows,
            output_root,
    ...
    for bucket_name, class_rows in rows_by_class.items():
        ...
        if class_image_pair_rows:
            ...
            # 类别级 image_pair energy_spectrum_mse 图源 NPY 也强制保存，不受 IS_SAVE_NPY 影响。
            save_energy_spectrum_mse_compare_npy(
