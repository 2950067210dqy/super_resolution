...
class AllHandlePipeline:
    ...
    def apply_hist_legend(self, ax: plt.Axes, series: dict[str, tuple[np.ndarray, np.ndarray]]) -> None:
        ...
        for exp_key in ordered_keys:
            ...
            handles.append(
                patch_cls(
                    facecolor=color,
                    edgecolor=self.darken_color(color),
                    linewidth=float(getattr(self.cfg, "HIST_LEGEND_EDGE_LINE_WIDTH", 2.0)),
    ...
    def run_all(self) -> None:
        ...
        for group_index, group in enumerate(groups, start=1):
            ...
            steps = [
                ("energy_spectrum", self.plot_energy_spectrum),
                ("histograms", self.plot_histogram_bundle),
            ]
            if normalize_name(group.category_name) != "all":
                steps.extend(
                    [
                        ("error_maps", self.plot_error_map_bundle),
                        ("composites", self.plot_composite_bundle),
                    ]
                )
                if normalize_name(group.category_name) in ("tbl", "twcf"):
                    steps.append(("tbl_twcf_flow_uv", self.plot_tbl_twcf_flow_uv))
                if normalize_name(group.category_name) == "tbl":
...
{chr(10).join(row_xml)}
    ...
    def plot_overlay_histogram(
        ...
        for exp_key, (x, y) in self.sorted_hist_series(series):
            ...
            edge_color = self.darken_color(hist_color)
            ax.bar(
                x,
                y,
                width=width,
                ...
                linewidth=float(getattr(self.cfg, "HIST_EDGE_LINE_WIDTH", 1.45)),
                label=self.experiment_label(exp_key),
            )
            ax.plot(
                x,
    ...
    def plot_flow_u_epe_histogram(self, group: GroupContext) -> None:
        ...
        for ax, series, x_label, axis_kind in (
            ...
            for exp_key, (x, y) in self.sorted_hist_series(series):
                ...
                ax.plot(x, y, color=edge_color, linewidth=self.cfg.HIST_LINE_WIDTH)
            ax.set_xlabel(x_label)
