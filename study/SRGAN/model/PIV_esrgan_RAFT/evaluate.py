...
def _save_flow_error_visuals(pred_bchw: torch.Tensor, gt_bchw: torch.Tensor, out_dir: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    _save_error_histogram(du, out_dir / "delta_u_error.png", title_text=r"$\Delta u$ Error Distribution", xlabel=r"$\Delta u$ displacement [px]", color="#F4A142")
    _save_error_histogram(dv, out_dir / "delta_v_error.png", title_text=r"$\Delta v$ Error Distribution", xlabel=r"$\Delta v$ displacement [px]", color="#4C9F70")
    _save_error_histogram(dw, out_dir / "delta_w_error.png", title_text=r"$\Delta w$ Error Distribution", xlabel=r"$\Delta w$ displacement [px]", color="#8E6BBE")
