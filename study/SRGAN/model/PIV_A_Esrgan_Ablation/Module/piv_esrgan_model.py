...
class OriginalESRGANGenerator(nn.Module):
    """
    最基础 ESRGAN 生成器，用于消融模式 5。

    与上面的 PIV_A 生成器相比，这个版本刻意去掉了 PIV 专用改造：
    1. 不使用 3/5/7 多尺度 stem，只使用 ESRGAN 经典 9x9 首层卷积；
    2. 不使用 previous/next 双帧特征融合，forward() 每次只处理单帧；
    3. 上采样回到 PixelShuffle 路线，保留 ESRGAN/SRGAN 家族最常见的结构；
    4. 不做图像对一致性设计，让模式 5 可以单独观察“基础 ESRGAN + RAFT EPE”带来的影响。
    """

    def __init__(self, inner_chanel: int = 3, num_residual_blocks: int = 23, scale: int = 2):
        super().__init__()
        self.scale = int(scale)

        # ESRGAN/SRGAN 经典浅层特征提取：9x9 卷积给单帧较大感受野。
        self.conv1 = nn.Sequential(
            nn.Conv2d(inner_chanel, 64, kernel_size=9, stride=1, padding=4),
            nn.LeakyReLU(0.2, inplace=True),
        )

        # RRDB 主干保持 ESRGAN 基础生成器语义；num_residual_blocks 放到全局变量里，
        # 需要轻量化或复现实验时只改 global_class，不必改网络代码。
        self.residual_blocks = nn.Sequential(
            *[RRDB(64, growth_channels=32) for _ in range(int(num_residual_blocks))]
        )
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)

        # 两次 2x PixelShuffle，总上采样倍率为 4x。这里使用 ICNR 初始化，
        # 让 PixelShuffle 早期输出更接近平滑上采样，减少棋盘格伪影干扰消融判断。
        self.upsample1 = nn.Conv2d(64, 64 * self.scale * self.scale, kernel_size=3, stride=1, padding=1)
        self.shuffle1 = nn.PixelShuffle(self.scale)
        self.upsample2 = nn.Conv2d(64, 64 * self.scale * self.scale, kernel_size=3, stride=1, padding=1)
        self.shuffle2 = nn.PixelShuffle(self.scale)
        self.lrelu = nn.LeakyReLU(0.2, inplace=True)

        self.conv_out = nn.Conv2d(64, inner_chanel, kernel_size=9, stride=1, padding=4)
        self._init_subpixel_weights()

    def _init_subpixel_weights(self) -> None:
        """对两个 PixelShuffle 前的卷积做 ICNR 初始化，尽量减少消融初期的上采样伪影。"""
        icnr_(self.upsample1.weight, scale=self.scale, initializer=nn.init.kaiming_normal_)
        icnr_(self.upsample2.weight, scale=self.scale, initializer=nn.init.kaiming_normal_)
        if self.upsample1.bias is not None:
            nn.init.zeros_(self.upsample1.bias)
        if self.upsample2.bias is not None:
            nn.init.zeros_(self.upsample2.bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        first_features = self.conv1(x)
        residual_features = self.conv2(self.residual_blocks(first_features))
        x = first_features + residual_features
        x = self.lrelu(self.shuffle1(self.upsample1(x)))
        x = self.lrelu(self.shuffle2(self.upsample2(x)))
        return self.conv_out(x)


class OriginalESRGANDownSample(nn.Module):
    """
    基础 ESRGAN 判别器中的下采样块。

    Conv(stride=2) 负责降采样，BatchNorm + LeakyReLU 是 SRGAN/ESRGAN 判别器常见组合。
    这里不加 sigmoid，因为后续损失使用 BCEWithLogitsLoss / RaGAN logits。
    """
...
class OriginalESRGANDiscriminator(nn.Module):
    """
    最基础 ESRGAN 判别器，用于消融模式 1、4、5。

    输入仍沿用当前训练逻辑构造的三通道时序图：
    [prev_gray, next_gray, abs(next_gray - prev_gray)]。
    这样只替换判别器结构，不改变数据含义，保证消融对照只落在“判别器架构”上。
    """

    def __init__(self, inner_chanel: int = 3):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(inner_chanel, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            OriginalESRGANDownSample(64, 64),
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2, inplace=True),
            OriginalESRGANDownSample(128, 128),
            nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            OriginalESRGANDownSample(256, 256),
            nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
            OriginalESRGANDownSample(512, 512),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 返回 logits，不做 sigmoid；loss.py 里的 RaGAN/BCEWithLogitsLoss 会负责数值稳定的概率映射。
        return self.net(x)
...
class Discriminator(nn.Module):
    """
    PIV_A_Esrgan_Ablation 的时序 Attention U-Net 判别器包装类。
