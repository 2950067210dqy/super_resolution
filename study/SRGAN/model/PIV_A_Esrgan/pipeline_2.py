...
class ESRuRAFTPIVInferenceWrapper(nn.Module):
    ...
    def forward(self, input_lr_prev, input_lr_next, flowl0):
        ...
        # USE_RAFT=True 时走 Generator + RAFT 的推理路径；
        # USE_RAFT=False 时只输出 previous/next 两张 SR 图，用来统计纯超分辨率推理成本。
...
def _profile_esru_raft_piv_model(model, sample_batch, device, warmup=5, iters=20):
    ...
    return {
        ...
        # pipeline_2 只统计推理口径，因此写入 inference_* 字段；
        # training_step_* 字段保留为空字符串，和主 pipeline 的汇总表结构对齐。
        "inference_gpu_memory_usage_gb": float(peak_memory_mb) / 1024.0,
        "inference_flops_g": float(flops) / 1e9,
...
def main():
    ...
    for job in run_jobs:
        ...
        for SCALE in global_data.esrgan.SCALES:
            ...
            # data_type 只用于输出目录和日志区分：联合训练为 RAFT，纯超分为 SR。
            data_type = "RAFT" if global_data.esrgan.USE_RAFT else "SR"
            ...
            wandb.init(
                ...
                config={
                    ...
                    "USE_RAFT": global_data.esrgan.USE_RAFT,
                    "DISCRIMINATOR": "Temporal Attention U-Net",
                    "DISCRIMINATOR_BASE_CHANNELS": global_data.esrgan.DISCRIMINATOR_BASE_CHANNELS,
                    "DISCRIMINATOR_USE_MULTISCALE": global_data.esrgan.DISCRIMINATOR_USE_MULTISCALE,
                    "DISCRIMINATOR_SPECTRAL_NORM": global_data.esrgan.DISCRIMINATOR_SPECTRAL_NORM,
            ...
            ESRuRAFT_PIV_model = ESRuRAFT_PIV(inner_chanel=3,scale=SCALE,batch_size=global_data.esrgan.BATCH_SIZE).to(global_data.esrgan.device, non_blocking=True)
            ...
            # USE_RAFT=False 时模型内部不会实例化 piv_RAFT，因此这里也不能创建 RAFT optimizer。
            ESRuRAFT_PIV_model_RAFT_optimizeroptimizer = (
                torch.optim.AdamW(
                    ESRuRAFT_PIV_model.piv_RAFT.parameters(),
                    lr=global_data.esrgan.RAFT_LR,
            ...
            raft_scheduler = (
                torch.optim.lr_scheduler.ReduceLROnPlateau(
                    ...
                    patience=global_data.esrgan.RAFT_LR_patience_level,
                    min_lr=global_data.esrgan.RAFT_LR_min,
                )
                if global_data.esrgan.USE_RAFT
                else None
            )
            ...
            last_good_model_save_path = (
                ...
                f"ESRuRAFT_PIV_model_{global_data.esrgan.name}.pth"
            )
            # 轮数
            """
            训练 start
            ...
            for epoch in epoch_range:
                ...
                for i, batch in enumerate(train_progress_bar):
                    """按 USE_RAFT 自动选择“SR+RAFT 联合训练”或“只做 SR 训练”。"""
                    try:
                ...
                # 动态学习率 step：
                # - 联合训练时 RAFT scheduler 继续看 AEE；
                # - SR-only 时没有 AEE，G/D scheduler 改看图像能量谱误差，避免引用不存在的 RAFT 指标。
                if global_data.esrgan.USE_RAFT:
                    raft_scheduler.step(avg_val_aee)
                else:
                    g_scheduler.step(avg_val_energy_spectrum_mse)
                    d_scheduler.step(avg_val_energy_spectrum_mse)
                #保存动态学习率器
                ESRuRAFT_PIV_g_scheduler_save_path = f"{global_data.esrgan.OUT_PUT_DIR}/{class_name}/{data_type}/scale_{int(SCALE * SCALE)}/{global_data.esrgan.MODEL_DIR}/ESRuRAFT_PIV_g_scheduler_{global_data.esrgan.name}.pth"
                ESRuRAFT_PIV_d_scheduler_save_path = f"{global_data.esrgan.OUT_PUT_DIR}/{class_name}/{data_type}/scale_{int(SCALE * SCALE)}/{global_data.esrgan.MODEL_DIR}/ESRuRAFT_PIV_d_scheduler_{global_data.esrgan.name}.pth"
                ESRuRAFT_PIV_raft_scheduler_save_path = f"{global_data.esrgan.OUT_PUT_DIR}/{class_name}/{data_type}/scale_{int(SCALE * SCALE)}/{global_data.esrgan.MODEL_DIR}/ESRuRAFT_PIV_raft_scheduler_{global_data.esrgan.name}.pth"
