...
def _build_model(_scale):
    """
    创建 PIV_A_Esrgan_Ablation 分支的测试模型。
...
def _resolve_data_type():
    """
    PIV_A_Esrgan_Ablation 的输出目录名与训练阶段保持一致。
...
def main():
    """
    加载训练好的 PIV_A_Esrgan_Ablation 模型，并对 TEST_DATASETS 执行完整 test_all。
    """

    return run_pipeline_test(
        global_data=global_data,
        branch_name="PIV_A_Esrgan_Ablation",
